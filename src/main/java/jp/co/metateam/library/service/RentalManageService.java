...
public class RentalManageService {
    ...
    public void save(RentalManageDto rentalManageDto) throws Exception {
        try {
            //左辺：は名前を付けただけ
            /*①RentalManageテーブルから直接EmployeeIdは取れないから画面上に入力した情報から社員番号情報を見つけなければならない
            　
              rentalManageDto.getEmployeeId()は画面上に入力した社員番号を引数
            　orElse(null)は探したときに値が見つからないときは代わりにnullを代入してね
              ifでnullだった時にはエラーが出るように書いている
            */
            ...
            /*①上のAccount account = this.accountRepository.findByEmployeeId(rentalManageDto.getEmployeeId()).orElse(null);で
              getしたaccountをデータベースであるrentalManageのsetAccountクラスに引数としてsetしている
            */
